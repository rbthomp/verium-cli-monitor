...
def parse_summary_msg(host, summary):
        ...
        total = summary['accepted'] + summary['rejected'] if summary['accepted'] + summary['r ejected'] > 0 else 1
...
def apply_formatting(line, statinfo, hl):
        ...
        if statinfo[0] == True and hl == True:
                ...
                hosts_win.addstr("{0:>8.3f} H/m".format(statinfo[2]), curses.A_REVERSE) #! HP M
                hosts_win.addstr("   ", curses.A_REVERSE)
                hosts_win.addstr("{0:>6.2f}%".format(statinfo[3]), curses.A_REVERSE)   #! Sha re %
                hosts_win.addstr("   {0:>6}    {1:<8}  │ {2:>4}   ".format(
                        statinfo[4], statinfo[5], statinfo[6]), curses.A_REVERSE)
                hosts_win.addstr("{0:>5.1f}°C ".format(statinfo[7]), curses.A_REVERSE)  #! CP U Temp
